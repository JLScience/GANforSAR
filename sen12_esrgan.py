...
from keras.layers import Conv2D, LeakyReLU, Concatenate, Lambda, Add, Input
...
class ESRGAN():
    ...
    def make_generator(self):
        ...
        def conv_lrelu(inp, filters, f_size, use_act=False, alpha=0):
        ...
        outp = conv_lrelu(inp, filters=self.num_f, f_size=self.f_size)
        # save output of first convolution to add it to the output of the RRDB's:
        out1 = outp
        # RRDB's:
        for _ in range(self.num_rrdbs):
            outp = rrd_block(outp, filters=self.num_f, f_size=self.f_size, alpha=0.2, residual_scaling_factor=0.2)
        # convolution after RRDB's (no activation function):
        outp = conv_lrelu(outp, filters=self.num_f, f_size=self.f_size)
        ...
        outp = conv_lrelu(outp, filters=self.num_f, f_size=self.f_size, use_act=True, alpha=0.2)
